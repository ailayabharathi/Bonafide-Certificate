...
const fetchDepartmentStudents = async (department: string): Promise<ManagedUser[]> => {
...
const DepartmentStudentsPage = () => {
  ...
  const { data: students = [], isLoading } = useQuery<ManagedUser[]>({
    queryKey: ["departmentStudents", profile?.department],
    queryFn: () => fetchDepartmentStudents(profile!.department!),
  ...
  return (
    <>
      <DashboardLayout title="Department Students">
        <Card>
          <CardHeader>
